...
WFDB_FILE *wfdb_open(const char *s, const char *record, int mode)
{
	...
	if (strcmp(record, "-") == 0)
		...
		printf("\n before for wfdb_filename: %s", wfdb_filename);
		//在for循环中的某一步卡住
		for (c0 = wfdb_path_list; c0; c0 = c0->next) {
			...
			*/
			...
			buf[len] = 0;
			printf("\n wfdb_open flag for 倒数5");
			wfdb_asprintf(&buf, "%s%s", buf, r);
			if (!buf)
				continue;
			printf("\n wfdb_open flag for 倒数4");
			printf("\n wfdb_filename: %s", wfdb_filename);
			printf("\n buf: %s", buf);
			printf("\n s: %s", s);
