...
void wqrs(int argc, char** argv)
{
	...
	printf("\n wqrs flag next is annopen");
	//2019.10.22
	//char new_root[50] = "usr/local/database/";
	//char new_root[100] = "dashboard/mit-bih-arrhythmia-database-1.0.0/";
	char new_root[100] = "mit-bih-arrhythmia-database-1.0.0/";
	strcat(new_root, record);
	char* new_record = new_root;
	//
    //if (annopen(record, &a, 1) < 0) return;
	if (annopen(new_record, &a, 1) < 0) return;
	printf("\n wqrs flag pre is annopen");
